...
def start(message):
    ...
    if message.text and message.text.strip().lower() == '/start':
        ...
        with db_lock:
            ...
            _cur = _c.cursor()
            _cur.execute("SELECT lang FROM user_languages WHERE user_id=?", (user_id,))
            _row = _cur.fetchone()
            _c.close()
        if _row is None:
...
def handle_callback(call):
    ...
    if data.startswith("cvt1_"):
        from_cid = data[5:]
        from_sym = CRYPTO_LIST[from_cid].split('(')[1].replace(')', '').strip() if from_cid in CRYPTO_LIST else from_cid.upper()
        ...
        for cid in coins:
            if cid == from_cid:
                continue
            sym = CRYPTO_LIST[cid].split('(')[1].replace(')', '').strip() if cid in CRYPTO_LIST else cid.upper()
    ...
    if data.startswith("cvt2_"):
        ...
        from_sym = CRYPTO_LIST[from_cid].split('(')[1].replace(')', '').strip() if from_cid in CRYPTO_LIST else from_cid.upper()
        to_sym   = CRYPTO_LIST[to_cid].split('(')[1].replace(')', '').strip() if to_cid in CRYPTO_LIST else to_cid.upper()
    ...
    if data.startswith("cmp1_"):
        ...
        row = []
        for cid in filtered:
            sym = CRYPTO_LIST[cid].split('(')[1].replace(')', '').strip()
        ...
        if row:
            rows.append(row)
        sym1 = CRYPTO_LIST[cid1].split('(')[1].replace(')', '').strip()
        bot.answer_callback_query(call.id)
        try:
            bot.edit_message_text(
            ...
            if src and dst and src != dst:
                ...
                if result_val is not None:
                    ...
                    if src in ('usd', 'toman'):
                        ...
                        src_sym = CRYPTO_LIST[src].split('(')[1].replace(')', '').strip()
                    else:
                        src_sym = src.upper()
                    ...
                    if dst in ('usd', 'toman'):
                        ...
                        # Extract symbol from "🪙 Name (SYMBOL)" format
                        dst_sym = CRYPTO_LIST[dst].split('(')[1].replace(')', '').strip()
                    else:
                        dst_sym = dst.upper()
        ...
        if amt_m:
            ...
            if cur and cur in CRYPTO_LIST:
                ...
                if p:
                    v_usd = amt * p
                    v_irr = v_usd * irr
                    name  = CRYPTO_LIST[cur].split('(')[1].replace(')', '').strip()
                    toman_lbl = T(uid, 'toman_label')
...
def _finalize_alert(message, user_id, crypto_id, direction_raw, price_raw):
    ...
    symbol = CRYPTO_LIST[crypto_id].split('(')[1].replace(')', '').strip()
...
def _do_compare(message, raw1, raw2, user_id: int = 0):
    ...
    for raw in (raw1, raw2):
        ...
        if cid not in CRYPTO_LIST:
            ...
            return
        ids.append(cid)
        names.append(CRYPTO_LIST[cid].split('(')[1].replace(')', '').strip())
        ...
        for pattern in patterns:
            ...
            if match:
                ...
                if result is not None:
                    src_sym = CRYPTO_LIST.get(src, src).split('(')[1].replace(')', '').strip() if src in CRYPTO_LIST else src.upper()
                    dst_sym = CRYPTO_LIST.get(dst, dst).split('(')[1].replace(')', '').strip() if dst in CRYPTO_LIST else dst.upper()
                    if dst == 'usd':
